...
describe("Integration", function () {
  ...
  describe("random-number", function () {
    ...
    beforeEach(async function () {
      ...
      const VRFCoordinatorMockDeployer = new ethers.ContractFactory(VRFCoordinatorMockABI, VRFCoordinatorMockBytecode, signer)
      const VRFCoordinatorMock = await VRFCoordinatorMockDeployer.deploy(linkToken.address)
      await VRFCoordinatorMock.deployTransaction.wait()

      const randomNumberDeployer = new ethers.ContractFactory(RandomNumberConsumerABI, RandomNumberConsumerBytecode, signer)
      randomNumberContract = await randomNumberDeployer.deploy(VRFCoordinatorMock.address, linkToken.address, KEY_HASH, FEE)
      await randomNumberContract.deployTransaction.wait()

      const transferTx = await linkToken.transfer(randomNumberContract.address, FUND_AMOUNT)
      await transferTx.wait()
    })

    it('Should throw an error if an invalid price consumer contract address is passed', function () {
      return assert.isRejected(this.hre.readRandomNumber(this.hre, "dummy address"))
    })
